...
class TestXetraETLMethods(TestCase):
    """
    ...
    _s3_bucket_name_src = "src-bucket"
    _s3_bucket_name_trg = "trg-bucket"
    _meta_key = "meta_key"
    ...
    def setUp(self):
        ...
        self._s3.create_bucket(
            Bucket=self._s3_bucket_name_src,
            CreateBucketConfiguration={"LocationConstraint": "eu-central-1"}
        )
        self._s3.create_bucket(
            Bucket=self._s3_bucket_name_trg,
            CreateBucketConfiguration={"LocationConstraint": "eu-central-1"}
        )
    ...
    def test_extract_files(self):
        """
        Tests the extract method when there are files to be extracted
        """
        df_expected = self._df_src.loc[1:8].reset_index(drop=True)
    ...
    def test_transform_report1_emptydf(self):
        """
        Tests the transform_report1 method with an empty DataFrame as input argument
        """
        log_expected = "The dataframe is empty. No transformations will be applied."
        extract_date = "2021-04-17"
    ...
    def test_transform_report1_ok(self):
        ...
        """
        log1_expected = "Applying transformations to Xetra source data for report 1 started..."
        log2_expected = "Applying transformations to Xetra source data finished..."
        ...
        df_input = self._df_src.loc[1:8].reset_index(drop=True)
        with patch.object(
            MetaProcess,
            "return_date_list",
            return_value=[extract_date, extract_date_list]
            ...
            with self.assertLogs() as logm:
                df_result = xetra_etl.transform_report1(df_input)
                self.assertIn(log1_expected, logm.output[0])
                self.assertIn(log2_expected, logm.output[1])
            self.assertTrue(df_expected.equals(df_result))
    ...
    def test_load(self):
        ...
        """
        log1_expected = "Xetra target data successfully written."
        log2_expected = "Xetra meta file successfully updated."
        df_expected = self._df_report
        meta_expected = ["2021-04-17", "2021-04-18", "2021-04-19"]
        extract_date = "2021-04-17"
        extract_date_list = [
            "2021-04-16",
            "2021-04-17",
        ...
        with patch.object(
            ...
            with self.assertLogs() as logm:
                ...
                self.assertIn(log1_expected, logm.output[1])
                self.assertIn(log2_expected, logm.output[4])
        trg_file = (
            self._s3_bucket_trg
        ...
        self._trg_bucket.delete_objects(
            Delete={
                "Objects": [
                    {"Key": trg_file},
                    {"Key": trg_file},
...
if __name__ == "__main__":
    ...
    # suite = TestSuite()
    # suite.addTest(TestXetraETLMethods("test_extract_files"))
